...
- Mapping:
  ...
  - `seven_day_sonnet` → legacy model-specific weekly window; retired `seven_day_opus` values are ignored.
  - `limits` → explicitly scoped additional weekly windows.
...
- API calls (all include `Cookie: sessionKey=<value>`):
  ...
  - `GET https://claude.ai/api/organizations/{orgId}/usage` → session, weekly, Sonnet, and scoped model limits.
