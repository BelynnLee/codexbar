...
> [!NOTE]
> This document describes the upstream macOS registry. The Windows port currently registers 41 of
> 60 tracked targets (59 upstream IDs plus the Windows-only OpenCode Zen extension). Treat the
> provider parity matrix in this directory as the source of truth for Windows coverage; automated
> Rust tests keep its registered source modes and strategy kinds aligned with runtime code.
